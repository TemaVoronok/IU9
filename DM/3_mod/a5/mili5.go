...
func (a *auto) Split1() ([]*pos, int) {
	...
	nodes := make([]*pos, n)
	for i := 0; i < n; i++ {
		nodes[i] = &pos{depth: 0, q: i}
		nodes[i].parent = nodes[i]
	}
	m := n
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			q1, q2 := nodes[i], nodes[j]
			if q1.Find() != q2.Find() {
				eq := true
				for k := 0; k < a.m; k++ {
...
func (a *auto) AufenkampHohn() auto {
	...
	return auto{q: Q1, m: a.m, d: d1, f: f1, q0: newq0}
}

func getAuto(reader *bufio.Reader) *auto {
	var (
		n, m, q, a int
		b          string
	)
...
func Equal(a, b auto) bool {
	if len(a.d) != len(b.d) || a.m != b.m {
		return false
	}
	for i := 0; i < len(a.d); i++ {
		for j := 0; j < a.m; j++ {
